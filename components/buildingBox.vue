...
import type {BuildingItem} from "~/types/options";
...
let list = ref<BuildingItem[]>([]);

const selectedOption = computed({
  get: () => {
    return list.value.find(option => option.code === props.modelValue) || null;
  },
  set: (option: BuildingItem | null) => {
    emit('update:modelValue', option ? option.code : null); // Use option.value
  }
});
...
const selectOption = (option: BuildingItem) => {
...
const getArea = async () => {
  let res = await useRequest<BuildingItem[]>('/wxh5/common/getBuildingList', {
    query: {
      area_id: props.areaId
    }
  });
  if (res.data) {
    list.value = res.data as BuildingItem[];
  }
}
